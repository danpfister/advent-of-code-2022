...
def draw_cave(paths):
    cave = np.full((10, 10), '.') # first index is top to bottom, second index is left to right 
    for path in paths:
...
if __name__ == "__main__":
    inputfile = open(r".\input\14_test.txt", 'r')
    inputdata = np.asarray([line.strip() for line in inputfile])
    paths = [[[int(corner[1]), int(corner[0]) - 494] for corner in re.findall(re.compile(r'(\d+),(\d+)(?: -> )*'), path)] for path in inputdata]
